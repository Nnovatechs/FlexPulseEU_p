...
begin
  ...
  if v_survey_id is null then
    ...
    set
      updated_at = coalesce((p_survey->>'updated_at')::timestamptz, timezone('utc', now())),
      published_at = coalesce((p_survey->>'published_at')::timestamptz, s.published_at),
      default_language = p_survey->>'default_language',
      supported_languages = array(select jsonb_array_elements_text(p_survey->'supported_languages')),
      definition_json = p_survey->'definition_json',
      mapping_contract_json = p_survey->'mapping_contract_json',
      mapping_compiled_json = p_survey->'mapping_compiled_json',
      mapping_hash = p_survey->>'mapping_hash',
      measurement_hash = p_survey->>'measurement_hash'
